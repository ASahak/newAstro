...
@Component({
  selector: 'app-milky-way-page',
  templateUrl: './milky-way-page.component.html',
  styleUrls: ['./milky-way-page.component.css']
})
export class MilkyWayPageComponent implements OnInit {
    styleForm;
  ...
  onChangeAddMilkyWay({value}:{value:MilkyWay}){
    value.publication = (new Date()).getTime();
    this.resetService.addNewMilkyWay(value);
}

 todosRecets(){
    this.resetService.getAllRecetsMilkyWay().subscribe(milkyWays =>this.milkyWays = milkyWays);
  }

onClickDelete(id){
  if(confirm("Ցանկանում եք հեռացնել տվյալը անվերարարձ՞")){
      this.resetService.deleteRecetMilkyWay(id);
  }
}
}

...
@Component({
  selector: 'app-rivers-all-continents',
  templateUrl: './rivers-all-continents.component.html',
  styleUrls: ['./rivers-all-continents.component.css'],
  animations: [
        trigger('fadeIn', [
            transition(':enter', [
                style({ opacity: '0' }),
...
export class RiversAllContinentsComponent implements OnInit {
...
setInputFocusOut(event:Event): void {
  ...
  search(query:string){
    this.searchClient = (query) ? this.rivers.filter(river => river.name.toLowerCase().includes(query.toLowerCase())):this.rivers;
  }
    onChangeAddRivers({value}:{value:Rivers}){
        value.url = this.imageURL;
        console.log(value)
        value.publication = (new Date()).getTime();
        this.statesService.addNewRivers(value)
        this.isImage = false;
        this.imageURL = '';
    }
   ...
   onInitContinentBool(){
     ...
     todosRecets(){
        ...
        this.statesService.getAllRecetsContinentsRivers().subscribe(rivers =>this.rivers = rivers);
      }
    ...
    onClickDelete(id){
      if(confirm("Ցանկանում եք հեռացնել տվյալը անվերարարձ՞?")){
          this.statesService.deleteRecetRivers(id);
      }
    }
}
